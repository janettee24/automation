...
def run_scrape() -> dict:
    ...
    for c in COMPETITORS:
        ...
        time.sleep(1)
        reviews = get_reviews_data(slug) if SCRAPERS_AVAILABLE else {}
        time.sleep(1)
        ads     = get_google_ads_data(slug, max_ads=10) if ADS_AVAILABLE else {}

        results[name] = {**landing, **reviews, 'ads_data': ads, 'competitor': name}
    return {
        'data'        : results,
...
with st.sidebar:
    ...
    ads_badge     = '🟢 Ads scraper ready' if ADS_AVAILABLE else '🟡 Ads scraper not found'
    st.markdown(f'<div style="font-size:0.8rem;color:#718096;margin-bottom:0.3rem;">{scraper_badge}</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="font-size:0.8rem;color:#718096;margin-bottom:0.8rem;">{ads_badge}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
# PAGE 3B — ADS INTELLIGENCE
# ══════════════════════════════════════════════════════════════════
elif page == "📣 Ads Intelligence":
    st.markdown('<h1 style="font-size:1.6rem;font-weight:700;color:#e2e8f0;margin-bottom:0.25rem;">Ads Intelligence</h1>', unsafe_allow_html=True)
    st.markdown('<div style="font-size:0.85rem;color:#718096;margin-bottom:2rem;">Google Ads Transparency Center · Real-time</div>', unsafe_allow_html=True)

    if not ADS_AVAILABLE:
        st.warning("⚠️ `scraper_google_ads.py` chưa được tìm thấy. Upload file lên cùng thư mục với `app.py`.")
        st.code("pip install Google-Ads-Transparency-Scraper", language="bash")
        st.stop()

    # Summary metrics
    all_ads  = {name: d.get('ads_data', {}) for name, d in latest.items()}
    total_ads = sum(a.get('ad_count', 0) for a in all_ads.values())
    active    = sum(1 for a in all_ads.values() if a.get('ad_count', 0) > 0)

    m1, m2, m3 = st.columns(3)
    with m1:
        st.markdown(f'<div class="card"><div class="metric-label">Total Ads</div><div class="metric-value">{total_ads}</div><div class="metric-sub">across all competitors</div></div>', unsafe_allow_html=True)
    with m2:
        st.markdown(f'<div class="card"><div class="metric-label">Active Advertisers</div><div class="metric-value" style="color:#68d391;">{active}</div><div class="metric-sub">of {len(comp_names)}</div></div>', unsafe_allow_html=True)
    with m3:
        top_ads = max(all_ads.items(), key=lambda x: x[1].get('ad_count', 0), default=('—', {}))
        st.markdown(f'<div class="card"><div class="metric-label">Most Active</div><div class="metric-value" style="font-size:1.1rem;padding-top:0.4rem;">{top_ads[0]}</div><div class="metric-sub">{top_ads[1].get("ad_count",0)} ads</div></div>', unsafe_allow_html=True)

    # Per-competitor breakdown
    st.markdown('<div class="section-title">Breakdown per Competitor</div>', unsafe_allow_html=True)

    for comp in comp_names:
        ads = latest.get(comp, {}).get('ads_data', {})
        ad_count  = ads.get('ad_count', 0)
        formats   = ads.get('ad_formats', {})
        keywords  = ads.get('top_keywords', [])
        messages  = ads.get('top_messages', [])
        adv_id    = ads.get('advertiser_id', '')
        error     = ads.get('error', '')

        status = '🟢' if ad_count > 0 else ('🔴' if error else '⚪')

        with st.expander(f"{status}  {comp}  ·  {ad_count} ads", expanded=ad_count > 0):
            if error and ad_count == 0:
                st.markdown(f'<div style="font-size:0.82rem;color:#fc8181;">⚠️ {error}</div>', unsafe_allow_html=True)
            elif ad_count == 0:
                st.markdown('<div style="font-size:0.82rem;color:#718096;">Không tìm thấy ads — có thể không chạy Google Ads hoặc bị block.</div>', unsafe_allow_html=True)
            else:
                col1, col2 = st.columns(2)
                with col1:
                    # Ad formats breakdown
                    st.markdown('<div style="font-size:0.72rem;color:#4a5568;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.5rem;">Ad Formats</div>', unsafe_allow_html=True)
                    for fmt, count in formats.items():
                        if count > 0:
                            pct  = int(count / max(sum(formats.values()), 1) * 100)
                            color = {'Text':'#63b3ed','Image':'#68d391','Video':'#f6c90e'}.get(fmt,'#a0aec0')
                            st.markdown(f'''<div style="margin-bottom:0.5rem;">
                                <div style="display:flex;justify-content:space-between;margin-bottom:2px;">
                                    <span style="font-size:0.8rem;color:#a0aec0;">{fmt}</span>
                                    <span style="font-size:0.8rem;color:{color};">{count} ({pct}%)</span>
                                </div>
                                <div class="score-bar-bg"><div style="width:{pct}%;height:8px;background:{color};border-radius:4px;"></div></div>
                            </div>''', unsafe_allow_html=True)

                with col2:
                    # Top keywords
                    if keywords:
                        st.markdown('<div style="font-size:0.72rem;color:#4a5568;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.5rem;">Top Keywords in Ads</div>', unsafe_allow_html=True)
                        kw_html = ' '.join(f'<span style="background:#1e2535;color:#a0aec0;padding:2px 8px;border-radius:12px;font-size:0.75rem;margin:2px;display:inline-block;">{k}</span>' for k in keywords[:6])
                        st.markdown(f'<div>{kw_html}</div>', unsafe_allow_html=True)

                # Sample messages
                if messages:
                    st.markdown('<div style="font-size:0.72rem;color:#4a5568;text-transform:uppercase;letter-spacing:0.08em;margin:0.8rem 0 0.4rem;">Sample Ad Copy</div>', unsafe_allow_html=True)
                    for msg in messages[:3]:
                        st.markdown(f'<div style="font-size:0.82rem;color:#cbd5e0;background:#1e2535;padding:0.5rem 0.8rem;border-radius:6px;margin-bottom:0.4rem;border-left:3px solid #4299e1;">"{msg[:120]}"</div>', unsafe_allow_html=True)

                # Transparency Center link
                if adv_id:
                    tc_url = f'https://adstransparency.google.com/advertiser/{adv_id}'
                    st.markdown(f'<div style="margin-top:0.8rem;"><a href="{tc_url}" target="_blank" style="font-size:0.78rem;color:#63b3ed;">🔗 View on Google Ads Transparency Center →</a></div>', unsafe_allow_html=True)

    # Cross-competitor keyword analysis
    st.markdown('<div class="section-title">Category-wide Ad Keywords</div>', unsafe_allow_html=True)
    all_kw: dict = {}
    for ads in all_ads.values():
        for kw in ads.get('top_keywords', []):
            all_kw[kw] = all_kw.get(kw, 0) + 1

    if all_kw:
        sorted_kw = sorted(all_kw.items(), key=lambda x: x[1], reverse=True)[:12]
        kw_html = ' '.join(
            f'<span style="background:{"#2d2500" if c>1 else "#1e2535"};color:{"#f6c90e" if c>1 else "#718096"};border:1px solid {"#b7791f" if c>1 else "#2d3748"};padding:4px 12px;border-radius:16px;font-size:0.8rem;margin:3px;display:inline-block;">{k} <span style="opacity:0.6;">×{c}</span></span>'
            for k, c in sorted_kw
        )
        st.markdown(f'<div class="card" style="line-height:2;">{kw_html}</div>', unsafe_allow_html=True)
        st.markdown('<div style="font-size:0.72rem;color:#4a5568;">🟡 Highlighted = used by 2+ competitors — potential overused words for Variant B</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="card" style="color:#4a5568;text-align:center;padding:2rem;">Chưa có ads data — scrape sẽ chạy tự động khi app load.</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    if st.button("⚡ Generate A/B Variants", type="primary"):
        ...
        # Kết hợp overused words từ landing page + Google Ads keywords
        ...
        # Thêm Google Ads keywords vào phân tích
        for d in latest.values():
            for kw in d.get('ads_data', {}).get('top_keywords', []):
