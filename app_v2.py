...
def load_data():
    # Thử live_data.json trước (từ Colab), fallback mock_data.json
    for fname in ['live_data.json', 'mock_data.json']:
        if os.path.exists(fname):
            ...
            if 'snapshots' in raw and isinstance(raw['snapshots'], list):
                # live_data format: flat list of snapshots
                if raw['snapshots'] and 'competitor' in raw['snapshots'][0]:
                    return raw, fname
                # mock_data format: list of week objects
                else:
                    return raw, fname
    return None, None
...
if not data:
    st.error("⚠️ Không tìm thấy data file. Upload live_data.json hoặc mock_data.json vào cùng thư mục.")
    st.stop()

# ── Parse data helper ─────────────────────────────────────────────
def get_latest_per_competitor(data):
    """Lấy snapshot mới nhất của mỗi competitor"""
    snaps = data.get('snapshots', [])
    latest = {}

    # Handle live_data format (flat list)
    if snaps and 'competitor' in snaps[0]:
        for s in snaps:
            comp = s['competitor']
            if comp not in latest or s['date'] > latest[comp]['date']:
                latest[comp] = s

    # Handle mock_data format (weekly objects)
    elif snaps and 'data' in snaps[0]:
        last_week = snaps[-1]
        for comp, d in last_week['data'].items():
            latest[comp] = {**d, 'competitor': comp, 'date': last_week['week']}

    return latest

def get_all_changes(data):
    """Lấy tất cả changes đã detect"""
    snaps = data.get('snapshots', [])
    changes = []

    # mock_data format
    if snaps and 'changes' in snaps[0]:
        for s in snaps:
...
with st.sidebar:
    ...
    page = st.radio("", ["📊 Overview", "🏆 Score Leaderboard", "🔄 Changes Timeline", "🔎 Competitor Detail", "🧪 A/B Generator"], label_visibility="collapsed")

    st.markdown("---")
    st.markdown('<div class="nav-label">Data source</div>', unsafe_allow_html=True)
    src_badge = '🟢 Live data' if source_file == 'live_data.json' else '🟡 Mock data'
    st.markdown(f'<div style="font-size:0.82rem;color:#718096;">{src_badge}<br>Updated: {last_updated}<br>{len(competitors)} competitors tracked</div>', unsafe_allow_html=True)

    if source_file == 'mock_data.json':
        st.markdown('<div style="font-size:0.75rem;color:#4a5568;margin-top:0.5rem;font-style:italic;">Upload live_data.json từ Colab để dùng real-time data</div>', unsafe_allow_html=True)
...
if page == "📊 Overview":
    ...
    for comp in competitors:
        ...
        model_badge = f'<span class="badge-ok">PLG</span>' if model == 'PLG' else f'<span class="badge-low">Sales-led</span>' if model == 'Sales-led' else f'<span class="badge-low">Hybrid</span>'
...
# ══════════════════════════════════════════════════════════════════
    ...
    if not scored:
        st.info("Chưa có score data. Chạy Cell 10 trong Colab notebook để generate scores.")
    else:
        medals = ['🥇','🥈','🥉','4️⃣','5️⃣']
        ...
        for comp, d in scored:
            ...
            scores_d = detail.get('scores', {}) if detail else {}
            notes_d  = detail.get('notes', {})  if detail else {}
            if not scores_d:
                continue
...
# ══════════════════════════════════════════════════════════════════
    ...
    filtered = [c for c in reversed(all_changes)
                if c.get('competitor','') in filter_comp
                and c.get('severity','low') in filter_sev]
...
# ══════════════════════════════════════════════════════════════════
    ...
    if scores_d:
        ...
        criteria_labels = {'clarity':'Clarity','value_prop':'Value Prop','cta_strength':'CTA Strength',
                           'social_proof':'Social Proof','pricing_transparency':'Pricing','audience_clarity':'Audience'}
        cols = st.columns(3)
...
# ══════════════════════════════════════════════════════════════════
    ...
    if generate or ab_variants:
        ...
        # Use pre-generated or generate on the fly
